...
async def add_question_paper(qn_paper: utils.CreateQuestionPaper, db: Session = Depends(get_db)):
    # create a model
...
async def read_qn_paper_by_name(qn_paper: utils.GetQnPaperByName, db: Session = Depends(get_db)):
...
async def read_qn_paper_by_semester(qn_paper: utils.GetQnPaperBySemester, db: Session = Depends(get_db)):
    qn_papers = db.query(models.QuestionPaperModel) \
